...
const SubscriberDashboard = ({ userData, onPay, announcements, tickets, repairs }) => {
     ...
     const handleSelectPlan = async (plan) => {
        if(confirm(`Do you want to apply for the ${plan.name}?`)) {
           try {
              ...
              await addDoc(collection(db, 'artifacts', appId, 'public', 'data', TICKETS_COLLECTION), { userId: userData.uid, username: userData.username, subject: 'New Subscription Application', message: `Applicant ${userData.username} (${userData.email}) has applied for the ${plan.name} plan. Please approve and assign an account number.`, status: 'open', adminReply: '', isApplication: true, targetUserId: userData.uid, targetPlan: plan.name, date: new Date().toISOString() });
  ...
  const handleApplyPlan = (planName) => { if(confirm(`Apply for ${planName}?`)) { const msg = `Requesting plan change.\n\nCurrent: ${userData.plan}\nNew: ${planName}`; const submitPlanTicket = async () => { setTicketLoading(true); try { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', TICKETS_COLLECTION), { userId: userData.uid, username: userData.username, subject: 'Plan Change Request', message: msg, status: 'open', adminReply: '', date: new Date().toISOString() }); alert("Application submitted!"); setActiveTab('support'); } catch(e) { alert("Failed."); } setTicketLoading(false); }; submitPlanTicket(); } };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 h-fit">
              ...
              <div className="space-y-4">
                ...
                {announcements && announcements.length > 0 ? announcements.map((ann) => (<div key={ann.id} className="flex items-start p-4 bg-slate-50 rounded-xl"><div className={`p-2.5 rounded-full mr-4 flex-shrink-0 ${getBgColor(ann.type)}`}>{getIcon(ann.type)}</div><div><p className="font-bold text-sm text-slate-700">{ann.title}</p><p className="text-xs text-slate-500 mt-0.5">{ann.message}</p><p className="text-[10px] text-slate-400 mt-1">{new Date(ann.date).toLocaleDateString()}</p></div></div>)) : <div className="flex items-center justify-center p-4 bg-slate-50 rounded-xl text-slate-400 text-sm">No new announcements.</div>}
              </div>
            </div>
          </div>
        </>
...
// 4. Admin Dashboard (Same as previous)
const AdminDashboard = ({ subscribers, announcements, payments, tickets, repairs }) => {
  // ... Reuse AdminDashboard logic from previous steps ...
  ...
  const [isCreatingUser, setIsCreatingUser] = useState(false);
   ...
   const handleApproveApplication = async (ticket) => {
      ...
      try {
         await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', COLLECTION_NAME, ticket.targetUserId), { status: 'active', accountNumber: newAccountNo, plan: planName, balance: 1500, dueDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() });
         await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', TICKETS_COLLECTION, ticket.id), { status: 'resolved', adminReply: `Approved! Account Number: ${newAccountNo}. Please proceed to payment.` });
              ...
              {tickets && tickets.length > 0 ? tickets.map(ticket => (
                <div key={ticket.id} className={`p-5 rounded-xl shadow-sm border ${ticket.isApplication ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-200'}`}>
                   <div className="flex justify-between items-start mb-3">
                      <div><h4 className="font-bold text-lg text-slate-800">{ticket.subject} {ticket.isApplication && <span className="bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded-full ml-2">APPLICATION</span>}</h4><p className="text-xs text-slate-500">From: <span className="font-bold text-blue-600">{ticket.username}</span> • {new Date(ticket.date).toLocaleString()}</p></div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
             <div className="overflow-x-auto">
               <table className="w-full text-left text-sm">
                 <thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr><th className="px-6 py-4 font-bold">User</th><th className="px-6 py-4 font-bold">Plan</th><th className="px-6 py-4 font-bold">Balance</th><th className="px-6 py-4 font-bold">Status</th></tr></thead>
                 <tbody className="divide-y divide-slate-100">
                   {filteredSubscribers.map((sub) => (
                     <tr key={sub.id}>
                       <td className="px-6 py-4"><div>{sub.username}</div><div className="text-xs text-slate-500">{sub.accountNumber === 'PENDING' ? <span className="text-orange-500 font-bold">PENDING APPROVAL</span> : sub.accountNumber}</div></td>
                       <td className="px-6 py-4">{sub.plan || '-'}</td>
                       <td className="px-6 py-4">${sub.balance?.toFixed(2)}</td>
                       <td className="px-6 py-4"><span className={`px-2 py-1 rounded-full text-xs font-bold capitalize ${sub.status === 'active' ? 'bg-green-100 text-green-700' : sub.status === 'applicant' ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700'}`}>{sub.status}</span></td>
                     </tr>
                   ))}
                 </tbody>
               </table>
...
// 5. Main App Logic (Same as previous)
export default function App() {
  ...
  const handlePayment = async (id, refNumber) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', COLLECTION_NAME, id), { balance: 0, status: 'active', lastPaymentRef: refNumber, lastPaymentDate: new Date().toISOString() });
  ...
  return (
    <Layout user={user} onLogout={handleLogout}>
      {user.role === 'admin' ? (
        ...
        <SubscriberDashboard userData={mySubscriberData || {}} onPay={handlePayment} announcements={announcements} tickets={tickets} repairs={repairs} />
      )}
    </Layout>
  );
}