    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      {activeTab === 'repairs' && (
         <div className="space-y-6">
            ...
            <div className="space-y-4">
               {repairs && repairs.filter(r => r.status !== 'Completed').length > 0 ? repairs.filter(r => r.status !== 'Completed').map(repair => (
                  <RepairStatusCard key={repair.id} repair={repair} isSubscriber={false} technicians={technicians} onAssign={handleAssignTech} />
               )) : null}
            </div>
            <div className="pt-8 border-t border-slate-200">
               <h3 className="text-lg font-bold text-slate-700 mb-4 flex items-center gap-2"><Clock size={18}/> Repair History</h3>
               {repairs && repairs.filter(r => r.status === 'Completed').length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{repairs.filter(r => r.status === 'Completed').map(repair => (<RepairStatusCard key={repair.id} repair={repair} isSubscriber={false} />))}</div>
            ...
            {(!repairs || repairs.length === 0) && <div className="text-center py-10 text-slate-400">No repair records found.</div>}
         </div>
      )}
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            <div className="space-y-4">
               ...
               )) : <div className="text-center py-20 bg-white rounded-2xl border border-slate-200"><CheckCircle2 size={48} className="mx-auto text-green-300 mb-3" /><p className="text-slate-500">All assigned repairs completed!</p></div>}
            </div>
        </div>
    );
};
